...
export const PageNav: React.FC<Props> = ({ title }) => {
  ...
  return (
    <div className="flex-col w-[300px] pt-4 pl-12 xl:pl-16 pr-0 pb-6 min-w-pageNav hidden lg:flex">
      {headers.length !== 0 && (
        <aside className="sticky top-12 h-[100vh] relative">
          <div className="overflow-y-auto h-full">
            <h5 className="text-sm text-gray-900 font-medium mb-3">
              On This Page
            </h5>
...
const nestingTw: Record<number, string> = {
  0: "ml-0",
  1: "ml-6",
  2: "ml-10",
};
      ...
      {headers.map((h, i) => {
        ...
        return (
          <React.Fragment key={`${h.id}-${i}`}>
            <li key={h.id} className={nestingTw[nesting]}>
              <Link
                ref={el => {
                  linkRefs[h.id] = el;
                }}
                className={classNames(
                  "inline-block text-gray-600 text-sm hover:text-pink-500",
                  isActive && "font-medium text-pink-500",
                )}
                href={`#${h.id}`}
